...
Please create [Issue](https://github.com/nholuongut/test-reporter/issues/new) or contribute with PR.
...
jobs:
  build-test:
    ...
    steps:
      ...
      - name: Test Report
        uses: nholuongut/test-reporter@v1
...
Test run must be configured to use [JSON](https://github.com/nholuongut/test/blob/master/pkgs/test/doc/configuration.md#reporter) reporter.
...
- [test configuration](https://github.com/nholuongut/test/blob/master/pkgs/test/doc/configuration.md)
</details>
...
- Check Runs are created for specific commit SHA. It's not possible to specify under which workflow test report should belong if more
  ...
  For more information, see [#67](https://github.com/nholuongut/test-reporter/issues/67).

## See also
- [paths-filter](https://github.com/nholuongut/paths-filter) - Conditionally run actions based on files modified by PR, feature branch, or pushed commits
